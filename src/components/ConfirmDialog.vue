<template>
  <div 
    v-if="isOpen"
    class="fixed inset-0 bg-black/70 flex items-center justify-center z-50 p-6 animate-fade-in"
    @click.self="cancel"
  >
    <div class="bg-[#1a1a1d] rounded-2xl border border-white/10 w-full max-w-md shadow-2xl animate-scale-in">
        ...
        <label class="flex items-center gap-3 cursor-pointer group">
          <div class="relative">
            ...
            <div class="w-5 h-5 rounded-md border-2 border-white/15 bg-white/5 peer-checked:bg-white peer-checked:border-white transition-all duration-75 flex items-center justify-center">
              <svg v-if="dontAskAgain" class="w-3 h-3 text-gray-900" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="p-6 border-t border-white/10 flex items-center justify-end gap-3">
        <button @click="cancel" class="px-6 py-2.5 bg-white/5 hover:bg-gray-800/80 text-white rounded-xl transition-all duration-75">Отмена</button>
        <button @click="confirm" class="px-6 py-2.5 bg-white hover:bg-gray-100 text-gray-900 rounded-xl transition-all duration-75">Подтвердить</button>
      </div>
    </div>
  </div>
</template>
