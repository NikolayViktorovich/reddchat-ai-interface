    ...
    <div class="glass-modal w-full md:max-w-5xl h-[90vh] md:rounded-2xl rounded-t-3xl border-t md:border border-white/5 flex flex-col md:flex-row overflow-hidden animate-slide-up md:animate-scale-in">
      ...
      <div class="md:hidden flex-1 flex flex-col overflow-hidden">
        <div v-if="!result" class="flex-1 overflow-y-auto p-4 space-y-4">
          <div @click="$refs.mobileInput.click()" class="rounded-2xl p-8 text-center border-2 border-dashed border-white/5 active:border-white/40 transition-fast">
        ...
        <div v-if="!result" class="p-4 border-t border-white/5">
          <button @click="analyze" :disabled="!files.length || loading" class="w-full py-3 rounded-2xl text-sm transition-fast" :class="files.length && !loading ? 'bg-white/10 text-white border border-white/5' : 'bg-white/5 text-gray-500 border border-white/5'">{{ loading ? 'Анализ...' : 'Начать' }}</button>
        </div>
      </div>
      ...
      <div class="hidden md:flex w-96 flex-col border-r border-white/5 p-6">
        ...
        <div class="flex-1 overflow-y-auto scrollbar-thin space-y-4">
          <div @dragover.prevent="drag = true" @dragleave.prevent="drag = false" @drop.prevent="handleDrop" @click="$refs.fileInput.click()" class="rounded-2xl p-6 text-center cursor-pointer border-2 border-dashed transition-fast" :class="drag ? 'border-white/40 bg-white/5' : 'border-white/5'">
          ...
          <div v-if="files.length" class="space-y-2">
            <div v-for="(f, i) in files" :key="i" class="flex items-center justify-between p-3 bg-white/5 rounded-xl border border-white/5 animate-fade-in">
              <div class="flex-1 min-w-0"><p class="text-white text-sm truncate">{{ f.name }}</p><p class="text-gray-500 text-xs">{{ formatSize(f.size) }}</p></div>
        ...
        <button @click="analyze" :disabled="!files.length || loading" class="w-full py-3 rounded-3xl mt-4 transition-fast" :class="files.length && !loading ? 'bg-white/10 text-white border border-white/5' : 'bg-white/5 text-gray-500 border border-white/5'">{{ loading ? 'Анализ...' : 'Начать' }}</button>
      </div>
      ...
      <div class="hidden md:flex flex-1 flex-col">
        ...
        <div class="flex-1 overflow-y-auto scrollbar-thin p-6">
          ...
          <div v-else class="space-y-6 animate-fade-in">
            ...
            <div class="bg-white/5 rounded-2xl p-6 border border-white/5"><h4 class="text-white text-lg mb-4">Содержание</h4><p class="text-gray-300 text-sm">{{ result.summary }}</p></div>
            <div class="bg-white/5 rounded-2xl p-6 border border-white/5"><h4 class="text-white text-lg mb-4">Темы</h4><div class="flex flex-wrap gap-2"><span v-for="(t, i) in result.topics" :key="i" class="px-3 py-1.5 bg-white/5 text-gray-300 text-sm rounded-full">{{ t }}</span></div></div>
          </div>
        </div>
      </div>
    </div>
