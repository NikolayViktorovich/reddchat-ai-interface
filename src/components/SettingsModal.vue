<template>
  <transition name="fade">
    ...
    >
      <div class="bg-[#1a1a1d] w-full md:max-w-lg md:rounded-2xl rounded-t-3xl border-t md:border border-white/10 shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-4 md:p-6 space-y-6">
              ...
              >
                ...
                <div class="flex-1">
                  <div class="flex items-center gap-2 mb-1">
                    ...
                    <span v-if="model.recommended" class="px-2 py-0.5 bg-white/10 text-gray-300 text-xs rounded">
                      Рекомендуется
                    </span>
                  </div>
          ...
          <div>
            ...
            <select
              ...
              class="w-full px-4 py-3 bg-white/5 border border-white/10 text-white rounded-xl focus:outline-none"
            >
        ...
        <div class="p-4 md:p-6 border-t border-white/10 flex gap-3">
          <button
            @click="$emit('close')"
            class="flex-1 py-3 bg-white/5 text-white rounded-2xl text-sm hover:bg-white/10"
          >
            Отмена
          </button>
          <button
            @click="saveSettings"
            class="flex-1 py-3 bg-white/10 text-white rounded-2xl text-sm border border-white/20 hover:bg-white/15"
          >
            Сохранить
          </button>
        </div>
