    ...
    <div class="bg-[#1a1a1d] w-full md:max-w-5xl h-[90vh] md:rounded-2xl rounded-t-3xl border-t md:border border-white/10 flex flex-col md:flex-row overflow-hidden animate-slide-up md:animate-scale-in">
      ...
      <div class="md:hidden flex-1 flex flex-col overflow-hidden">
        <div v-if="!showMobileResults" class="flex-1 overflow-y-auto p-4 space-y-4">
          <div>
            ...
            <textarea v-model="prompt" placeholder="Опишите изображение..." class="w-full h-28 px-4 py-3 bg-white/5 text-white placeholder-gray-500 rounded-2xl border border-white/10 focus:outline-none resize-none text-sm"></textarea>
          </div>
          <div>
            <label class="text-sm text-gray-400 mb-2 block">Стиль</label>
            <div class="grid grid-cols-2 gap-2">
              <button v-for="s in styles" :key="s.value" @click="selectedStyle = s.value" class="px-4 py-2.5 rounded-xl text-sm transition-fast" :class="selectedStyle === s.value ? 'bg-white/10 text-white border border-white/20' : 'bg-white/5 text-gray-400 border border-white/10'">{{ s.label }}</button>
            </div>
          </div>
        ...
        <div v-else class="flex-1 flex flex-col overflow-hidden">
          ...
          <div class="flex-1 overflow-y-auto p-4">
            ...
            <div v-else class="grid grid-cols-2 gap-3">
              <div v-for="(img, i) in generatedImages" :key="i" class="bg-white/5 rounded-xl overflow-hidden border border-white/10 animate-fade-in" @click="preview = img">
      ...
      <div class="hidden md:flex w-96 flex-col border-r border-white/10 p-6">
        ...
        <div class="flex-1 overflow-y-auto scrollbar-thin space-y-4">
          <div>
            <label class="text-sm text-gray-400 mb-2 block">Описание</label>
            <textarea v-model="prompt" placeholder="Опишите изображение..." class="w-full h-32 px-4 py-3 bg-white/5 text-white placeholder-gray-500 rounded-2xl border border-white/10 focus:outline-none resize-none text-sm"></textarea>
          </div>
          <div>
            <label class="text-sm text-gray-400 mb-2 block">Стиль</label>
            <div class="grid grid-cols-2 gap-2">
              <button v-for="s in styles" :key="s.value" @click="selectedStyle = s.value" class="px-4 py-2 rounded-xl text-sm transition-fast" :class="selectedStyle === s.value ? 'bg-white/10 text-white border border-white/20' : 'bg-white/5 text-gray-400 border border-white/10'">{{ s.label }}</button>
            </div>
          </div>
        </div>
        <button @click="generate" :disabled="!prompt.trim() || isGenerating" class="w-full py-3 rounded-3xl mt-4 transition-fast" :class="prompt.trim() && !isGenerating ? 'bg-white/10 text-white border border-white/20' : 'bg-white/5 text-gray-500 border border-white/10'">{{ isGenerating ? 'Генерация...' : 'Создать' }}</button>
      </div>
      ...
      <div class="hidden md:flex flex-1 flex-col">
        ...
        <div class="flex-1 overflow-y-auto scrollbar-thin p-6">
          ...
          <div v-else class="grid grid-cols-2 gap-4">
            <div v-for="(img, i) in generatedImages" :key="i" class="group bg-white/5 rounded-2xl overflow-hidden border border-white/10 cursor-pointer animate-fade-in" @click="preview = img">
              <img :src="img.url" :alt="img.prompt" class="w-full h-auto" loading="lazy" />
            </div>
          </div>
        </div>
