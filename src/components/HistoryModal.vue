    ...
    <div v-if="isOpen" class="fixed inset-0 bg-black/80 flex items-end md:items-center justify-center z-50 md:p-6" @click.self="$emit('close')">
      ...
      <div class="md:hidden flex-1 flex flex-col overflow-hidden">
        <div class="p-3 border-b border-white/10">
          <div class="relative">
            ...
            <input v-model="searchQuery" type="text" placeholder="Поиск..." class="w-full pl-11 pr-4 py-2.5 bg-white/5 text-white text-sm placeholder-white/40 rounded-full border border-white/10 focus:outline-none focus:border-white/20" />
          </div>
        </div>
        ...
        <Transition name="slide-right" mode="out-in">
          <div v-if="!mobileSelectedConv" key="list" class="flex-1 overflow-y-auto">
            <div v-if="filteredConversations.length === 0" class="flex flex-col items-center justify-center h-full p-6">
              <p class="text-white/40 text-sm">{{ searchQuery ? 'Ничего не найдено' : 'История пуста' }}</p>
            </div>
            <div v-else class="divide-y divide-white/10">
              <div v-for="conv in filteredConversations" :key="conv.id" class="flex items-center justify-between p-4 active:bg-white/5 transition-colors" @click="mobileSelectedConv = conv">
                <div class="flex-1 min-w-0 mr-3">
                  ...
                  <p class="text-white/40 text-xs mt-1">{{ formatDate(conv.createdAt) }} • {{ conv.messages.length }} сообщ.</p>
                </div>
          ...
          <div v-else key="detail" class="flex-1 flex flex-col overflow-hidden">
            <div class="flex items-center gap-3 p-4 border-b border-white/10">
              <button @click="mobileSelectedConv = null" class="p-1 text-white/50"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg></button>
              <div class="flex-1 min-w-0">
                <h3 class="text-white text-sm truncate">{{ mobileSelectedConv.title }}</h3>
                <p class="text-white/40 text-xs">{{ formatDate(mobileSelectedConv.createdAt) }}</p>
              </div>
            </div>
            <div class="flex-1 overflow-y-auto p-4 space-y-3">
              <div v-for="message in mobileSelectedConv.messages" :key="message.id" class="flex" :class="message.role === 'user' ? 'justify-end' : 'justify-start'">
                <div class="max-w-[85%] rounded-xl px-3 py-2 text-sm break-words" :class="message.role === 'user' ? 'bg-white/10 text-white whitespace-pre-wrap' : 'bg-white/5 text-white'" style="word-break: break-word; overflow-wrap: break-word;">
      ...
      <div class="hidden md:flex w-96 flex-col border-r border-white/10">
        <div class="p-5 border-b border-white/10">
          ...
          <p class="text-white/40 text-sm">Всего: {{ conversations.length }}</p>
        </div>
        <div class="p-4 border-b border-white/10">
          <div class="relative">
            <svg class="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input v-model="searchQuery" type="text" placeholder="Поиск..." class="w-full pl-11 pr-4 py-2 bg-white/5 text-white text-sm placeholder-white/40 rounded-full border border-white/10 focus:outline-none focus:border-white/20" />
          </div>
        </div>
        <div class="flex-1 overflow-y-auto scrollbar-thin p-3">
          <div v-if="filteredConversations.length === 0" class="text-center py-12"><p class="text-white/40 text-sm">{{ searchQuery ? 'Ничего не найдено' : 'История пуста' }}</p></div>
          <div v-else class="space-y-1">
            <div v-if="pinnedConversations.length" class="mb-4">
              <p class="text-xs text-white/40 mb-2 px-2 uppercase tracking-wide">Закреплено</p>
              <div v-for="conv in pinnedConversations" :key="conv.id" class="group relative bg-transparent hover:bg-white/5 rounded-lg cursor-pointer transition-all" :class="{ 'bg-white/5': conv.id === selectedConvId }" @click="selectedConvId = conv.id" @dblclick="openConversation">
                ...
                <div v-else class="flex items-center p-3">
                  <div class="flex-1 min-w-0">
                    ...
                    <p class="text-white/50 text-xs">{{ formatDate(conv.createdAt) }} • {{ conv.messages.length }} сообщ.</p>
                  </div>
                  <button @click.stop="toggleMenu(conv.id)" class="p-1.5 opacity-0 group-hover:opacity-100 hover:bg-white/10 rounded-lg transition-opacity flex-shrink-0 ml-2">
                    <svg class="w-4 h-4 text-white/50" fill="currentColor" viewBox="0 0 24 24"><circle cx="12" cy="6" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="18" r="1.5"/></svg>
                  </button>
                </div>
                <div v-if="openMenuId === conv.id" class="absolute right-0 top-full mt-1 w-44 bg-[#18181b] rounded-lg border border-white/10 shadow-xl z-50 overflow-hidden animate-dropdown">
                  <button @click.stop="startEdit(conv)" class="w-full flex items-center gap-3 px-3 py-2 text-sm text-white/60 hover:bg-white/5 hover:text-white"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>Переименовать</button>
                  <button @click.stop="handlePin(conv.id)" class="w-full flex items-center gap-3 px-3 py-2 text-sm text-white/60 hover:bg-white/5 hover:text-white"><svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24"><path d="M5 5v14l7-4 7 4V5a2 2 0 00-2-2H7a2 2 0 00-2 2z"/></svg>Открепить</button>
                  <button @click.stop="handleDelete(conv.id)" class="w-full flex items-center gap-3 px-3 py-2 text-sm text-white/60 hover:bg-white/5 hover:text-white"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>Удалить</button>
                </div>
              </div>
            </div>
            <div v-if="unpinnedConversations.length">
              <p v-if="pinnedConversations.length" class="text-xs text-white/40 mb-2 px-2 uppercase tracking-wide">Остальные</p>
              <div v-for="conv in unpinnedConversations" :key="conv.id" class="group relative bg-transparent hover:bg-white/5 rounded-lg cursor-pointer transition-all" :class="{ 'bg-white/5': conv.id === selectedConvId }" @click="selectedConvId = conv.id" @dblclick="openConversation">
                ...
                <div v-if="openMenuId === conv.id" class="absolute right-0 top-full mt-1 w-44 bg-[#252528] rounded-xl border border-white/10 shadow-xl z-50 overflow-hidden animate-dropdown">
                  <button @click.stop="startEdit(conv)" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-white/70 hover:bg-white/10 hover:text-white"><svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"/></svg>Переименовать</button>
                  <button @click.stop="handlePin(conv.id)" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-white/70 hover:bg-white/10 hover:text-white"><svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5v14l7-4 7 4V5a2 2 0 00-2-2H7a2 2 0 00-2 2z"/></svg>Закрепить</button>
                  <button @click.stop="handleDelete(conv.id)" class="w-full flex items-center gap-3 px-3 py-2.5 text-sm text-white/70 hover:bg-white/10 hover:text-white"><svg class="w-[18px] h-[18px]" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>Удалить</button>
                </div>
              </div>
            </div>
          </div>
      ...
      <div class="hidden md:flex flex-1 flex-col">
        <div class="flex items-center justify-between p-5 border-b border-white/10">
          <div v-if="selectedConversation"><h2 class="text-lg text-white mb-1">{{ selectedConversation.title }}</h2><p class="text-white/40 text-sm">{{ formatDate(selectedConversation.createdAt) }}</p></div>
          <div v-else><h2 class="text-lg text-white">Предпросмотр</h2></div>
          <button @click="$emit('close')" class="p-2 text-white/50 hover:text-white transition-colors"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
        <div class="flex-1 overflow-y-auto scrollbar-thin p-5">
          <div v-if="!selectedConversation" class="flex flex-col items-center justify-center h-full">
            <p class="text-white/40">Выберите диалог для просмотра</p>
          </div>
          <div v-else class="space-y-4">
            <div v-for="message in selectedConversation.messages" :key="message.id" class="flex" :class="message.role === 'user' ? 'justify-end' : 'justify-start'">
              <div class="max-w-2xl rounded-xl px-4 py-3 text-sm break-words" :class="message.role === 'user' ? 'bg-white/10 text-white whitespace-pre-wrap' : 'bg-white/5 text-white'" style="word-break: break-word; overflow-wrap: break-word;">
                <div v-if="message.role === 'assistant'" v-html="formatMarkdown(message.content)" class="markdown-content"></div>
                <span v-else>{{ message.content }}</span>
              </div>
            </div>
...
.markdown-content :deep(code) { font-family: 'JetBrains Mono', monospace; font-size: 0.85em; word-break: break-all; }
.markdown-content :deep(pre code) { background: transparent; padding: 0; color: #abb2bf; word-break: normal; }
...
.markdown-content :deep(a) { color: #61afef; word-break: break-all; }
</style>
